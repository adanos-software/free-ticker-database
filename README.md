...
|---|---:|---|
| Core listings | 61,692 | Rows in `data/core_listings.csv`; one collision-safe core row per security keyed by `listing_key`. |
| Primary tickers | 63,802 | Rows in `data/tickers.csv`; one primary row per security. |
...
| Stocks | 47,771 | Primary ticker rows where `asset_type=Stock`. |
...
| Sector/category coverage | 62,263 (97.6%) | Primary ticker rows with either `stock_sector` or `etf_category`. |
| Stock sector coverage | 46,263 | Primary ticker rows with a non-empty `stock_sector`. |
| ETF category coverage | 16,000 | Primary ticker rows with a non-empty `etf_category`. |
| Core listing-scope rows | 61,692 | Rows in `data/instrument_scopes.csv` where `instrument_scope=core`. |
| Core primary rows with ISIN | 61,002 | Core primary listing rows with an ISIN; tracked as `scope_reason=primary_listing`. |
...
| Extended listing-scope rows | 30,314 | Rows in `data/instrument_scopes.csv` where `instrument_scope=extended`. |
...
| Entry quality source-gap rows | 11,453 | Listing-keyed rows that are structurally valid but retain explicit source or metadata gaps. |
| Entry quality warn rows | 167 | Listing-keyed rows with deterministic warnings requiring review/allowlist coverage. |
