...
|---|---:|---|
| Core listings | 57,770 | Rows in `data/core_listings.csv`; one collision-safe core row per security keyed by `listing_key`. |
| Primary tickers | 63,208 | Rows in `data/tickers.csv`; one primary row per security. |
| Full listing rows | 74,758 | Rows in `data/listings.csv`; venue-level rows keyed by `listing_key`, including cross/secondary listings. |
| Stocks | 47,446 | Primary ticker rows where `asset_type=Stock`. |
| ETFs | 15,762 | Primary ticker rows where `asset_type=ETF`. |
...
| Core listing-scope rows | 57,770 | Rows in `data/instrument_scopes.csv` where `instrument_scope=core`. |
