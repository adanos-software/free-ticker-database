...
| Aliases | 122,058 | Rows in `data/aliases.csv`; structured alias/name/identifier lookup rows. |
| ISIN coverage | 59,953 (97.3%) | Primary ticker rows with a non-empty `isin`. |
...
| Core primary rows with ISIN | 53,211 | Core primary listing rows with an ISIN; tracked as `scope_reason=primary_listing`. |
