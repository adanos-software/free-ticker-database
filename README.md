...
| Primary tickers | 63,826 | Rows in `data/tickers.csv`; one primary row per security. |
| Full listing rows | 92,032 | Rows in `data/listings.csv`; venue-level rows keyed by `listing_key`, including cross/secondary listings. |
| Stocks | 47,779 | Primary ticker rows where `asset_type=Stock`. |
| ETFs | 16,047 | Primary ticker rows where `asset_type=ETF`. |
...
| Core primary rows missing ISIN | 717 | Core primary listing rows still missing ISIN; tracked as `scope_reason=primary_listing_missing_isin`. |
...
| Entry quality source-gap rows | 11,379 | Listing-keyed rows that are structurally valid but retain explicit source or metadata gaps. |
