...
| Primary tickers | 63,773 | Rows in `data/tickers.csv`; one primary row per security. |
...
| Aliases | 125,227 | Rows in `data/aliases.csv`; structured alias/name/identifier lookup rows. |
...
| Entry quality source-gap rows | 20,972 | Listing-keyed rows that are structurally valid but retain explicit source or metadata gaps. |
