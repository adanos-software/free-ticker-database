...
|---|---:|---|
| Core listings | 61,798 | Rows in `data/core_listings.csv`; one collision-safe core row per security keyed by `listing_key`. |
| Primary tickers | 63,911 | Rows in `data/tickers.csv`; one primary row per security. |
| Full listing rows | 92,102 | Rows in `data/listings.csv`; venue-level rows keyed by `listing_key`, including cross/secondary listings. |
| Stocks | 47,776 | Primary ticker rows where `asset_type=Stock`. |
...
| FIGI coverage | 65,314 | Listing-keyed rows in `data/identifiers_extended.csv` with OpenFIGI coverage. |
| Sector/category coverage | 62,292 (97.5%) | Primary ticker rows with either `stock_sector` or `etf_category`. |
...
| Core listing-scope rows | 61,798 | Rows in `data/instrument_scopes.csv` where `instrument_scope=core`. |
| Core primary rows with ISIN | 60,938 | Core primary listing rows with an ISIN; tracked as `scope_reason=primary_listing`. |
| Core primary rows missing ISIN | 860 | Core primary listing rows still missing ISIN; tracked as `scope_reason=primary_listing_missing_isin`. |
