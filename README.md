...
| Sector/category coverage | 61,420 (99.8%) | Primary ticker rows with either `stock_sector` or `etf_category`. |
| Stock sector coverage | 45,845 | Primary ticker rows with a non-empty `stock_sector`. |
...
| Entry quality source-gap rows | 5,997 | Listing-keyed rows that are structurally valid but retain explicit source or metadata gaps. |
