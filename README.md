...
|---|---:|---|
| Core listings | 61,693 | Rows in `data/core_listings.csv`; one collision-safe core row per security keyed by `listing_key`. |
| Primary tickers | 63,803 | Rows in `data/tickers.csv`; one primary row per security. |
...
| Stocks | 47,772 | Primary ticker rows where `asset_type=Stock`. |
| ETFs | 16,031 | Primary ticker rows where `asset_type=ETF`. |
...
| Aliases | 125,516 | Rows in `data/aliases.csv`; structured alias/name/identifier lookup rows. |
| ISIN coverage | 62,555 (98.0%) | Primary ticker rows with a non-empty `isin`. |
| FIGI coverage | 65,407 | Listing-keyed rows in `data/identifiers_extended.csv` with OpenFIGI coverage. |
| Sector/category coverage | 62,262 (97.6%) | Primary ticker rows with either `stock_sector` or `etf_category`. |
| Stock sector coverage | 46,262 | Primary ticker rows with a non-empty `stock_sector`. |
| ETF category coverage | 16,000 | Primary ticker rows with a non-empty `etf_category`. |
| Core listing-scope rows | 61,693 | Rows in `data/instrument_scopes.csv` where `instrument_scope=core`. |
| Core primary rows with ISIN | 61,003 | Core primary listing rows with an ISIN; tracked as `scope_reason=primary_listing`. |
| Core primary rows missing ISIN | 690 | Core primary listing rows still missing ISIN; tracked as `scope_reason=primary_listing_missing_isin`. |
| Extended listing-scope rows | 30,313 | Rows in `data/instrument_scopes.csv` where `instrument_scope=extended`. |
...
| Entry quality source-gap rows | 11,571 | Listing-keyed rows that are structurally valid but retain explicit source or metadata gaps. |
| Entry quality warn rows | 168 | Listing-keyed rows with deterministic warnings requiring review/allowlist coverage. |
...
Official source candidates and reconciled source gaps are tracked in [`data/masterfiles/source_candidates.json`](data/masterfiles/source_candidates.json) and summarized by [`data/reports/source_inventory_gap.md`](data/reports/source_inventory_gap.md). Current source coverage status: `5` missing current-scope exchanges, `6` parser todo rows, `0` real global-expansion candidates, `49` official-full exchanges, and `33` official-partial exchanges. Remaining work includes source-parser backlog plus field-completion and taxonomy coverage.
