...
| Primary tickers | 63,303 | Rows in `data/tickers.csv`; one primary row per security. |
...
| Stocks | 47,523 | Primary ticker rows where `asset_type=Stock`. |
...
| Aliases | 125,264 | Rows in `data/aliases.csv`; structured alias/name/identifier lookup rows. |
...
| Entry quality source-gap rows | 7,056 | Listing-keyed rows that are structurally valid but retain explicit source or metadata gaps. |
