...
| Full listing rows | 92,051 | Rows in `data/listings.csv`; venue-level rows keyed by `listing_key`, including cross/secondary listings. |
| Stocks | 47,770 | Primary ticker rows where `asset_type=Stock`. |
| ETFs | 16,097 | Primary ticker rows where `asset_type=ETF`. |
...
| Core primary rows missing ISIN | 807 | Core primary listing rows still missing ISIN; tracked as `scope_reason=primary_listing_missing_isin`. |
...
| Entry quality source-gap rows | 12,354 | Listing-keyed rows that are structurally valid but retain explicit source or metadata gaps. |
