...
| Sector/category coverage | 62,291 (97.6%) | Primary ticker rows with either `stock_sector` or `etf_category`. |
| Stock sector coverage | 46,267 | Primary ticker rows with a non-empty `stock_sector`. |
| ETF category coverage | 16,024 | Primary ticker rows with a non-empty `etf_category`. |
...
| Core primary rows missing ISIN | 714 | Core primary listing rows still missing ISIN; tracked as `scope_reason=primary_listing_missing_isin`. |
...
| Entry quality source-gap rows | 11,375 | Listing-keyed rows that are structurally valid but retain explicit source or metadata gaps. |
