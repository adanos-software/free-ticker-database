...
| Aliases | 122,020 | Rows in `data/aliases.csv`; structured alias/name/identifier lookup rows. |
| ISIN coverage | 59,953 (97.4%) | Primary ticker rows with a non-empty `isin`. |
...
| Core primary rows with ISIN | 53,210 | Core primary listing rows with an ISIN; tracked as `scope_reason=primary_listing`. |
| Core primary rows missing ISIN | 885 | Core primary listing rows still missing ISIN; tracked as `scope_reason=primary_listing_missing_isin`. |
...
| Entry quality source-gap rows | 7,288 | Listing-keyed rows that are structurally valid but retain explicit source or metadata gaps. |
