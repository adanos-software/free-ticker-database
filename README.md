...
|---|---:|---|
| Core listings | 57,615 | Rows in `data/core_listings.csv`; one collision-safe core row per security keyed by `listing_key`. |
| Primary tickers | 63,126 | Rows in `data/tickers.csv`; one primary row per security. |
...
| ETFs | 15,642 | Primary ticker rows where `asset_type=ETF`. |
...
| Aliases | 124,989 | Rows in `data/aliases.csv`; structured alias/name/identifier lookup rows. |
...
| Sector/category coverage | 63,125 (100.0%) | Primary ticker rows with either `stock_sector` or `etf_category`. |
| Stock sector coverage | 47,483 | Primary ticker rows with a non-empty `stock_sector`. |
| ETF category coverage | 15,642 | Primary ticker rows with a non-empty `etf_category`. |
| Core listing-scope rows | 57,615 | Rows in `data/instrument_scopes.csv` where `instrument_scope=core`. |
| Core primary rows with ISIN | 56,976 | Core primary listing rows with an ISIN; tracked as `scope_reason=primary_listing`. |
| Core primary rows missing ISIN | 639 | Core primary listing rows still missing ISIN; tracked as `scope_reason=primary_listing_missing_isin`. |
| Extended listing-scope rows | 16,977 | Rows in `data/instrument_scopes.csv` where `instrument_scope=extended`. |
...
| Entry quality source-gap rows | 6,302 | Listing-keyed rows that are structurally valid but retain explicit source or metadata gaps. |
