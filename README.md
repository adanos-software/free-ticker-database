...
|---|---:|---|
| Core listings | 61,749 | Rows in `data/core_listings.csv`; one collision-safe core row per security keyed by `listing_key`. |
| Primary tickers | 63,872 | Rows in `data/tickers.csv`; one primary row per security. |
| Full listing rows | 92,087 | Rows in `data/listings.csv`; venue-level rows keyed by `listing_key`, including cross/secondary listings. |
| Stocks | 47,796 | Primary ticker rows where `asset_type=Stock`. |
...
| Aliases | 125,576 | Rows in `data/aliases.csv`; structured alias/name/identifier lookup rows. |
| ISIN coverage | 62,555 (97.9%) | Primary ticker rows with a non-empty `isin`. |
...
| Core listing-scope rows | 61,749 | Rows in `data/instrument_scopes.csv` where `instrument_scope=core`. |
...
| Entry quality source-gap rows | 11,490 | Listing-keyed rows that are structurally valid but retain explicit source or metadata gaps. |
