...
| Primary tickers | 63,238 | Rows in `data/tickers.csv`; one primary row per security. |
| Full listing rows | 74,707 | Rows in `data/listings.csv`; venue-level rows keyed by `listing_key`, including cross/secondary listings. |
| Stocks | 47,502 | Primary ticker rows where `asset_type=Stock`. |
| ETFs | 15,736 | Primary ticker rows where `asset_type=ETF`. |
...
| Core primary rows missing ISIN | 754 | Core primary listing rows still missing ISIN; tracked as `scope_reason=primary_listing_missing_isin`. |
...
| Entry quality source-gap rows | 6,919 | Listing-keyed rows that are structurally valid but retain explicit source or metadata gaps. |
