...
|---|---:|---|
| Core listings | 61,698 | Rows in `data/core_listings.csv`; one collision-safe core row per security keyed by `listing_key`. |
| Primary tickers | 63,810 | Rows in `data/tickers.csv`; one primary row per security. |
...
| ISIN coverage | 62,562 (98.0%) | Primary ticker rows with a non-empty `isin`. |
...
| Core listing-scope rows | 61,698 | Rows in `data/instrument_scopes.csv` where `instrument_scope=core`. |
| Core primary rows with ISIN | 60,998 | Core primary listing rows with an ISIN; tracked as `scope_reason=primary_listing`. |
...
| Extended listing-scope rows | 30,308 | Rows in `data/instrument_scopes.csv` where `instrument_scope=extended`. |
...
| Entry quality source-gap rows | 11,581 | Listing-keyed rows that are structurally valid but retain explicit source or metadata gaps. |
| Entry quality warn rows | 181 | Listing-keyed rows with deterministic warnings requiring review/allowlist coverage. |
