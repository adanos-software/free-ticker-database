...
|---|---:|---|
| Core listings | 61,702 | Rows in `data/core_listings.csv`; one collision-safe core row per security keyed by `listing_key`. |
| Primary tickers | 63,813 | Rows in `data/tickers.csv`; one primary row per security. |
...
| Stocks | 47,781 | Primary ticker rows where `asset_type=Stock`. |
...
| Aliases | 125,531 | Rows in `data/aliases.csv`; structured alias/name/identifier lookup rows. |
| ISIN coverage | 62,565 (98.0%) | Primary ticker rows with a non-empty `isin`. |
...
| Core listing-scope rows | 61,702 | Rows in `data/instrument_scopes.csv` where `instrument_scope=core`. |
