...
| Stocks | 47,777 | Primary ticker rows where `asset_type=Stock`. |
| ETFs | 16,134 | Primary ticker rows where `asset_type=ETF`. |
...
| ISIN coverage | 62,515 (97.8%) | Primary ticker rows with a non-empty `isin`. |
| FIGI coverage | 65,323 | Listing-keyed rows in `data/identifiers_extended.csv` with OpenFIGI coverage. |
...
| Entry quality source-gap rows | 11,565 | Listing-keyed rows that are structurally valid but retain explicit source or metadata gaps. |
