...
|---|---:|---|
| Core listings | 57,725 | Rows in `data/core_listings.csv`; one collision-safe core row per security keyed by `listing_key`. |
| Primary tickers | 63,236 | Rows in `data/tickers.csv`; one primary row per security. |
| Full listing rows | 74,705 | Rows in `data/listings.csv`; venue-level rows keyed by `listing_key`, including cross/secondary listings. |
| Stocks | 47,501 | Primary ticker rows where `asset_type=Stock`. |
| ETFs | 15,735 | Primary ticker rows where `asset_type=ETF`. |
...
| Aliases | 125,090 | Rows in `data/aliases.csv`; structured alias/name/identifier lookup rows. |
| ISIN coverage | 61,764 (97.7%) | Primary ticker rows with a non-empty `isin`. |
| FIGI coverage | 65,733 | Listing-keyed rows in `data/identifiers_extended.csv` with OpenFIGI coverage. |
| Sector/category coverage | 63,122 (99.8%) | Primary ticker rows with either `stock_sector` or `etf_category`. |
...
| Core listing-scope rows | 57,725 | Rows in `data/instrument_scopes.csv` where `instrument_scope=core`. |
...
| Entry quality source-gap rows | 6,925 | Listing-keyed rows that are structurally valid but retain explicit source or metadata gaps. |
