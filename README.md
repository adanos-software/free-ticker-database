...
| Aliases | 125,525 | Rows in `data/aliases.csv`; structured alias/name/identifier lookup rows. |
| ISIN coverage | 62,530 (97.9%) | Primary ticker rows with a non-empty `isin`. |
| FIGI coverage | 65,352 | Listing-keyed rows in `data/identifiers_extended.csv` with OpenFIGI coverage. |
...
| Core primary rows with ISIN | 60,949 | Core primary listing rows with an ISIN; tracked as `scope_reason=primary_listing`. |
...
| Extended listing-scope rows | 30,319 | Rows in `data/instrument_scopes.csv` where `instrument_scope=extended`. |
...
| Entry quality source-gap rows | 11,442 | Listing-keyed rows that are structurally valid but retain explicit source or metadata gaps. |
