# Override Debt Report

Generated at: `2026-06-05T17:28:22Z`

## Summary

| Metric | Value |
|---|---:|
| metadata_present_rows | 21,376 |
| metadata_resolved_exact | 21,371 |
| metadata_resolved_canonical | 0 |
| metadata_partial_canonical | 0 |
| metadata_resolved_policy | 3 |
| metadata_partial_policy | 0 |
| metadata_superseded_by_isin_inference | 0 |
| metadata_open | 2 |
| alias_present_rows | 617 |
| alias_resolved | 617 |
| alias_open | 0 |

## Open Metadata By Field

- `isin`: `2`

## Open Alias Removals By Exchange

- None

## Open Metadata Samples

- `NYSE::ALX` `isin` target=`US0147521092` actual=``
- `NASDAQ::AMAL` `isin` target=`` actual=`US0226711010`

## Open Alias Samples

- None
