...
def test_validation_report_gates_source_gap_classifications():
        ...
        source_of_truth_decisions=[
            {
                ...
                "name": "Microsoft Corporation",
                "gap_class": "official_identifier_source_gap",
                "source_of_truth_outcome": "official_fill_required",
                "core_action": "keep_in_current_scope",
                "fill_action": "fill_from_source",
                "review_needed": "true",
                "decision_policy": "Require reviewed evidence.",
                "next_action": "Acquire source.",
                "source_gate": "Exact symbol/name and checksum.",
            },
            {
                "field": "missing_sector_stock",
    ...
    assert gates["source_of_truth_decision_invalid_rows"]["actual"] == 0
...
def test_validation_report_fails_stale_or_mismatched_source_of_truth_decisions():
    missing = ticker("MISS", stock_sector="", isin="")
    report = build_validation_report(
        tickers=[missing],
        listings=[listing(missing)],
        ...
        source_of_truth_decisions=[
            {
                ...
                "asset_type": "Stock",
                "name": "Microsoft Corporation",
                "gap_class": "different_class",
                "source_of_truth_outcome": "unresolved",
                "core_action": "keep_in_current_scope",
                "fill_action": "manual_review_required",
                "review_needed": "false",
                "decision_policy": "",
                "next_action": "",
                "source_gate": "Exact symbol/name and checksum.",
    ...
    gates = {gate["name"]: gate for gate in report["gates"]}
    assert report["passed"] is False
    assert gates["source_of_truth_decision_invalid_rows"]["actual"] == 1
    assert gates["source_of_truth_decision_duplicate_keys"]["actual"] == 1
    assert gates["source_of_truth_decision_gap_mismatch"]["actual"] == 1
    assert gates["source_of_truth_decision_class_mismatch"]["actual"] == 1
...
def test_validation_report_fails_untrimmed_listing_and_ticker_names():
    msft = ticker("MSFT")
    msft["name"] = "Microsoft Corporation\xa0"
    listed = listing(msft)
    ...
    report = build_validation_report(
        ...
        adanos_reference=[adanos_reference(ticker("MSFT"))],
    ...
    gates = {gate["name"]: gate for gate in report["gates"]}
    assert report["passed"] is False
    assert gates["untrimmed_ticker_name_count"]["actual"] == 1
    assert gates["untrimmed_listing_name_count"]["actual"] == 1
    assert "NASDAQ::MSFT" in gates["untrimmed_listing_name_count"]["details"][0]
