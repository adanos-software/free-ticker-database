...
def test_normalize_review_result_parses_batch_response_line(tmp_path):
    queue_payload = {
        "items": [
            {
                "ticker": "AAA",
    ...
    line = {
        "key": "blocked-alias-present--AAA--NASDAQ",
        "metadata": {"ticker": "AAA", "exchange": "NASDAQ"},
        "response": {
            "candidates": [
                {
                    "content": {
                                ...
                                "text": json.dumps(
                                    {
                                        ...
                                        "name_matches_listing": "yes",
                                        "alias_actions": [
                                            {"alias": "example", "decision": "remove", "reason": "bad alias"}
                                        ],
                                        "metadata_actions": [],
                                        "confidence": 0.95,
                                        "summary": "Remove the alias.",
                                    }
                                )
                            }
                        ]
    ...
    assert normalized["entry_decision"] == "fix_metadata"
    assert normalized["queue_total_score"] == 90
    assert normalized["alias_actions"][0]["decision"] == "remove"


def test_normalize_review_result_collects_errors(tmp_path):
    schema = {
        "required": ["ticker", "exchange", "entry_decision", "ticker_exists", "name_matches_listing", "alias_actions", "metadata_actions", "confidence", "summary"],
        "properties": {
            "entry_decision": {"enum": ["keep", "drop_entry", "fix_metadata", "needs_human"]},
            "ticker_exists": {"enum": ["yes", "no", "unknown"]},
            "name_matches_listing": {"enum": ["yes", "no", "unknown"]},
...
def test_build_normalized_payload_and_csv(tmp_path):
    normalized_items = [
        {
            "ticker": "AAA",
            "exchange": "NASDAQ",
