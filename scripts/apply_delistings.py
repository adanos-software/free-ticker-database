...
def classify_candidate(candidate: dict[str, str]) -> str:
    ...
    if classification == "delisted" and (
        exchange == "BSE_IN" or exchange in NASDAQ_DELETE_EXCHANGES
    ):
        return (
            "apply_drop_override"
...
def build_drop_row(candidate: dict[str, str]) -> dict[str, str]:
    return {
        ...
        "confidence": "0.99",
        "reason": (
            "Official delisting evidence passed the gated apply path; generated by "
            "scripts/apply_delistings.py from the delisting report."
        ),
    }
